...
export default class Afl_ArticleFeedbackForm extends LightningElement {
	...
	@track selectedValue;
	savedVote = '';
	activePositiveValues;
	activeNegativeValues;
	allValues;
	reasonTypeOptions;
	...
	async handleToggleLike() {
		...
		this.disliked = false;
		this.checkSameVote();
		await this.upsertSingleVote(this.recordId, this.liked, this.isSameVote, this.hasNoRate);
		this.setLikeValues();
		this.savedVote = '5';
	...
	handleUploadFinished(event) {
		...
		files.forEach((x) => {
			this.insertedFilesIds.push(x.documentId);
		});
	}
